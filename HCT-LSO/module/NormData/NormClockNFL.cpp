...
// the situation. 
norm_data::NormClockNFL::NormClockNFL(NormClockNFL && rhs) = default;
NormClockNFL & norm_data::NormClockNFL::operator=(NormClockNFL && rhs) = default;


void norm_data::NormClockNFL::insertData(Ethinicity race, Gender gender, const char * sector, float inclin,
...
void norm_data::NormClockNFL::insertData(Ethinicity race, Gender gender, const char * sector, float inclin, float inter, float stddev)
{
	auto& dataMap = getDataMap(race, gender);
...
int norm_data::NormClockNFL::getPercentile(Ethinicity race, Gender gender, int age, EyeSide side, int sector, float value)
{
	auto& dataMap = getDataMap(race, gender);
	if (dataMap.empty()) {
		return 0;
...
const char* norm_data::NormClockNFL::getSectorStr(EyeSide side, int sector)
{
	switch (sector) {
	case 0:
		return "SUP";
...
std::map<std::string, ClockNFL_Data>& norm_data::NormClockNFL::getDataMap(Ethinicity race, Gender gender)
{
...
NormClockNFL::NormClockNFLImpl & norm_data::NormClockNFL::getImpl(void) const
{
	return *d_ptr;
}
