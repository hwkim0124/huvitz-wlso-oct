...
namespace WsoToolkit
{
    ...
    public partial class OctScanModeWindow
    {
        ...
        private void RetreiveProtocolInitParam()
        {
            ...
            _octInitParam.previewType = WsoOctDefs.OctPatternType.MULTI_LINE;
        ...
        private void ReflectReportThicknessMap()
        {
            var tmap = new WsoOctScan.OctThicknessMapDescript();

            if (WsoOctUtil.IsStratumParamValid(_stratumParam))
            {
                if (OctAnalysis.GetThicknessMap(_reportId, _stratumParam, ref tmap))
                {
                    OnOctScanThicknessMapCaptured(tmap.data, tmap.width, tmap.height);
                }
            }
        }
        ...
        private void ReflectReportSectionChart(bool clear)
        {
            reportSectionChart.IsETDRSChart = true;
            reportSectionChart.ClearChart();
            ...
            if (!clear)
            {
                if (WsoOctUtil.IsStratumParamValid(_stratumParam))
                {
                    ...
                    {
                        reportSectionChart.IsETDRSChart = true;
                        var desc = new WsoOctScan.OctETDRSChartDescript();
                        if (OctAnalysis.GetETDRSChart(_reportId, _stratumParam, ref desc))
                        {
                            reportSectionChart.ETDRSDescript = desc;
                        }

                        if (_stratumParam.IsGccAnalysis())
                        {
                            var gcc_desc = new WsoOctScan.OctGCCThickChartDescript();
                            if (OctAnalysis.GetGCCThickChart(_reportId, _stratumParam, ref gcc_desc))
                            {
                            }
                        }
                    }
                    ...
                    {
                        reportSectionChart.IsClockChart = true;
                        var desc = new WsoOctScan.OctClockChartDescript();
                        if (OctAnalysis.GetClockChart(_reportId, _stratumParam, ref desc))
                        {
                            reportSectionChart.ClockDescript = desc;
                        }
                    }
                }
            }
            reportSectionChart.Refresh();
        }
        ...
        private void ReflectScanPatternSelected(bool init)
        {
            ...
            if (_scanPatterns.Length > 0)
            {
                ...
                if (init)
                {
                    ...
                    if (count > 0)
                    {
                        cmbScanPattern.SelectedIndex = (count > 4 ? 4 : 0);
                    }
                }
            }
        }
        ...
        private void ReflectScanOverlapsSelected()
        {
            ...
            if (GetScanPatternSelected(out pattern) && GetScanPointsSelected(out points) && GetScanLinesSelected(out lines))
            {
                ...
                if (count > 0)
                {
                    cmbScanOverlaps.SelectedIndex = 0;
                }
                return;
            }
        }
        ...
        private void ReflectScanRangeSelected()
        {
            ...
            if (GetScanPatternSelected(out pattern))
            {
                ...
                if (xcount > 0)
                {
                    cmbScanRangeX.SelectedIndex = (xcount > 2 ? 2 : 0);
                }
                if (ycount > 0)
                {
                    cmbScanRangeY.SelectedIndex = (ycount > 2 ? 2 : 0);
                }
                return;
            }
        }
        ...
        private void OnOctScanPreviewImageCaptured(nint data, int width, int height, float quality, float snr_ratio, int ref_point, int index_image)
        {
            ...
            // Count the true capture rate for the frames that actually update the preview,
            // so the reported frame rate reflects the capture rate rather than any throttling.
        ...
        private void OnOctScanThicknessMapCaptured(nint data, int width, int height)
        {
            if (data == 0) return;
            int length = width * height;
            ...
            Dispatcher.BeginInvoke(() =>
            {
                reportThicknessMap.CallbackOctThicknessMapCaptured(buff, width, height);
            }, DispatcherPriority.Normal);
        }
